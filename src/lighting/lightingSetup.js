...
export function setupLighting(scene) {
  ...
  const signLight = new THREE.SpotLight(0xd4a853, 2, 8, Math.PI / 6, 0.5);
  ...
  galleryScreenZPositions.forEach(z => {
    ...
    const leftSpot = new THREE.SpotLight(0xeeeeff, 1.5, 10, Math.PI / 5, 0.6);
    ...
    const rightSpot = new THREE.SpotLight(0xeeeeff, 1.5, 10, Math.PI / 5, 0.6);
  ...
  for (let z = -14; z > -36; z -= 6) {
    const galleryOverhead = new THREE.PointLight(0xdddde8, 0.5, 10);
  ...
  const corridorLight1 = new THREE.PointLight(0x6644aa, 0.8, 8);
  corridorLight1.position.set(0, 3.5, -39);
  scene.add(corridorLight1);

  const corridorLight2 = new THREE.PointLight(0x4466bb, 0.6, 8);
  ...
  const theaterRectLight = new THREE.RectAreaLight(0xaabbcc, 3, 12, 6.75);
  ...
  const theaterAmbient = new THREE.PointLight(0x111122, 0.2, 25);
  ...
  return {
    ...
    updateTheaterLight(color) {
      ...
      theaterRectLight.intensity = 3 + color.getHSL({}).l * 2; // brighter for bright scenes
    },
  };
}
